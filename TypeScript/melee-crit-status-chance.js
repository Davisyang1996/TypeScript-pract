...
const finalCriticalChanceGalSteel = roundTo(100 * baseCriticalChance * (1 + galvanizedSteelBonus + rivenCritBonus + bloodRushBonus), 1); // final critical chance calculation with galvanized steel
const finalCriticalChanceSacSteel = roundTo(100 * baseCriticalChance * (1 + sacrificialSteelBonus + rivenCritBonus + bloodRushBonus), 1); // final critical chance calculation with sacrificial steel
const finalHeavyCriticalChanceGalSteel = roundTo(100 * baseCriticalChance * (1 + galvanizedSteelBonus * 2 + rivenCritBonus + bloodRushHeavyBonus), 1); // final heavy attack critical chance calculation with double Galvanized Steel mod bonus and blood rush bonus based on heavy combo tier
const finalHeavyCriticalChanceSacSteel = roundTo(100 * baseCriticalChance * (1 + sacrificialSteelBonus * 2 + rivenCritBonus + bloodRushHeavyBonus), 1); // final heavy attack critical chance calculation with double Sacrificial Steel mod bonus and blood rush bonus based on heavy combo tier
const finalStatusChance = roundTo(100 * baseStatusChance * (1 + galvanizedElementalistBonus + weepingWoundsBonus + rivenStatusBonus + bonusFrom6060Mods), 1); // final status chance calculation
